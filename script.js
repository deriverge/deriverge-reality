...
function filterProperties() {
    ...
    const priceMin = parseInt(document.getElementById('filter-price-min').value, 10) || 0;
    const priceMax = parseInt(document.getElementById('filter-price-max').value, 10) || Infinity;
  
    const properties = document.querySelectorAll('.property');
    properties.forEach(property => {
      const propertyType = property.getAttribute('data-type');
      const propertyPrice = parseInt(property.getAttribute('data-price'), 10);
  
      if ((type === 'all' || propertyType === type) && propertyPrice >= priceMin && propertyPrice <= priceMax) {
